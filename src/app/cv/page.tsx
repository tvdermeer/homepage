  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-16 sm:px-6">
      <header className="mb-12 border-b border-slate-800 pb-12">
        <h1 className="text-4xl font-bold tracking-tight text-white">Thomas van der Meer</h1>
        ...
        <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-400">
          ...
          <a href="mailto:Tvdermeer@gmail.com" className="hover:text-white transition-colors">
            Tvdermeer@gmail.com
          </a>
      ...
      <section className="mb-12">
        ...
        <div className="space-y-8">
          {experiences.map((exp) => (
            <div key={exp.company + exp.period} className="border-l-2 border-slate-800 pl-6">
              <div className="flex flex-col justify-between gap-1 sm:flex-row sm:items-center">
                <h3 className="text-lg font-semibold text-white">{exp.role}</h3>
                <span className="text-sm text-slate-500">{exp.period}</span>
              </div>
              <p className="mt-1 font-medium text-emerald-400">{exp.company}</p>
      ...
      <section className="mb-12">
        ...
        <div className="space-y-6">
          {education.map((edu) => (
            <div key={edu.degree} className="border-l-2 border-slate-800 pl-6">
              <div className="flex flex-col justify-between gap-1 sm:flex-row sm:items-center">
                <h3 className="text-lg font-semibold text-white">{edu.degree}</h3>
                <span className="text-sm text-slate-500">{edu.year}</span>
              </div>
              <p className="mt-1 text-slate-400">{edu.institution}</p>
              <p className="mt-1 text-sm text-slate-500">{edu.details}</p>
            </div>
          ))}
        </div>
      </section>
            ...
            <div key={skillGroup.category}>
              ...
              <div className="flex flex-wrap gap-2">
                {skillGroup.items.map((skill) => (
                  <span
                    key={skill}
                    className="rounded-lg bg-slate-900 px-3 py-1.5 text-sm font-medium text-slate-300 border border-slate-800"
                  >
                    {skill}
                  </span>
                ))}
